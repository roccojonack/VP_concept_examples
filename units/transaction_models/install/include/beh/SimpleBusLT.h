...
public:
  ...
  typedef tlm_utils::simple_target_socket_tagged<SimpleBusLT, BUSWIDTH>    target_socket_type;
  typedef tlm_utils::simple_initiator_socket_tagged<SimpleBusLT, BUSWIDTH> initiator_socket_type;

public:
  std::vector<target_socket_type> target_socket;
  std::vector<initiator_socket_type> initiator_socket;
...
public:
  ...
  {
    for (unsigned int i = 0; i < InitSingleton->getNumberOfInstances(); ++i) {
                           ...
                           sc_core::sc_time& t)
  {
    ...
    assert(portId < InitSingleton->getNumberOfInstances());
